...
$codepay_config['return_url'] = $codepay_config['path'] . '/ok.php'; //自动生成跳转地址


$codepay_config['notify_url'] = $codepay_config['path'] . '/ok1.php'; //自动生成通知地址 优先级最高不传入则为系统设置里设置



if (empty($_POST)) $_POST = $_GET; //如果为GET方式访问
if (get_usershipin('price',$_POST['id'])<0)
{

    alert_href('金额不能低于0元','/pay.php?id='.$_REQUEST['id']);

}
$sql_select = "select dingdanhao from ".flag."order where dingdanhao = '".$_POST['out_trade_no']."'";
...
if(!$row)
{
    ...
    $_data1['vid'] = $_POST['id'];
    $_data1['uid'] =get_usershipin('uid',$_POST['id']);
    $_data1['dingdanhao'] = $_POST['out_trade_no'];
    $_data1['vprice'] = get_usershipin('price',$_POST['id']);
...
$user = get_usershipin('uid',$_REQUEST['id']);//提交的用户名

$price = (float)$_POST["total_fee"]; //提交的价格
...
if ($price <= 0) $price = (float)$_POST["total_fee"]; //如果没提供自定义输入金额则使用money参数

$price = number_format($price, 2, '.', '');// 四舍五入只保留2位小数。

if (empty($codepay_config) || (int)$codepay_config['id'] <= 1) exit('未设置ID密钥');

$pay_id = $_POST["out_trade_no"]; //网站唯一标识 需要充值的用户名，用户ID或者自行创建订单 建议传递用户的ID
