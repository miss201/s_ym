...
<style type="text/css">
	...
	<body style="max-width:460px; margin:0 auto;">
	 <div style="height: 42px; width: 100%; background-color: #000;float: left;display: block;opacity: 0.8; line-height: 42px;font-size: 22px;text-align: center;color: #FFF;margin-bottom: 8px;">รางวัลเมฆแท้</div>
 ...
 <div  style="height:7PX"></div>
    ...
    <div  align="center"  style="font-size:15PX"><h2>↓↓↓ 视 频 推 荐 ↓↓↓</h2></div>
    <div  style="height:7PX"></div>
    <hr>
	...
	<div class="h_playlist_box" id="article_list"> 
	   ...
	   <?php                                                                                
 						...
 						$result = mysql_query('select * from '.flag.'usershipin  where uid ='.get_usershipin('uid',$_GET['id']).'  and id !=   '.$_GET['id'].'  and isdel =0   order by ID asc   limit 10  ');
						while($row = mysql_fetch_array($result)){
						?>
	
	 <a href="?id=<?=$row['ID']?>"><dl   ><dt style="height: 9rem;width: 100%;"><em></em><img src="<?=$row['image']?>"    height="240px"  style=" display: inline;"  ></dt>    <dd>  <span><?=$row['name']?></span>   </dd></dl> </a>    
	 <? }?>
