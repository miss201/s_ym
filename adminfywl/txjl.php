      	...
      	if($n_zt != 2)
        {
        	$rmb = $_GET['rmb'];
     		$uid = $_GET['uid'];
          	...
          	mysql_query("update ".flag."user set rmb = {$rmb} where ID = {$uid}");
...
//-->
  ...
  <button type="submit" class="btn btn-default">搜索</button>
                                                                       ...
                                                                       <?php 
  	$result = mysql_query("select sum(rmb-sxf) as total from ".flag."tx where zt = 1");
    ...
    $today = date('Y-m-d');           
    echo $today;                                
    $result1 = mysql_query("select sum(rmb-sxf) as total from ".flag."tx where zt = 1 and date like '{$today}%'");
  	$row1 = mysql_fetch_array($result1);
                                   
	$total = $row['total']?:0;
    $total_today = $row1['total']?:0;      
    $total_today = round($total_today,2);    
	 $total = round($total,2); 
  ?>
                              <div class="form-group">
    <label for="exampleInputName2">提现总额：<span style="color:red"><?php echo $total ?></span>元</label>
  </div>      
                                                          <div class="form-group">
    <label for="exampleInputName2">今天提现总额：<span style="color:red"><?php echo $total_today ?></span>元</label>
  </div>      
</form>       
 ...
 $sql = 'select * from '.flag.'tx where zt  =  '.$_GET['type'].'   order by ID desc , id desc';
						 ...
						 {$type='<font color="red">拒绝</font>';}
 							?>
									...
									<td><?=$row['date']?>  </td>                                   
									<td>
								   <a class="btn" href="?zt=1&id=<? echo $row['ID']?>" >通过</a>	 <a class="btn" href="?zt=2&id=<? echo $row['ID']?>&uid=<? echo $row['uid']?>&rmb=<? echo $row['rmb']?>&n_zt=<? echo $row['zt']?>" >拒绝</a>	
