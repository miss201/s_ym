...
if($_GET['act'] =='del'){
	$sql = 'delete from '.flag.'user where id = '.$_GET['id'].'';
	if(mysql_query($sql)){
		alert_href('删除成功!','daili.php');
	}else{
		alert_back('删除失败！');
	}
}
...
/*如果要获取全部数值则使用下面代码*/
   $id= implode(",",$_POST['id']);
   $str="DELETE FROM ".flag."user where id in ($id)";
   mysql_query($str);
  echo "<script>alert('删除成功！');window.location.href='daili.php';</script>";
}
}					?>
<!DOCTYPE html>
<html>
...
<br>
  ...
  <div class="form-group">
    <a href="daili_add.php" class="btn btn-primary">新增</a>
    <label for="exampleInputName2">关键字</label>
    <input type="text" name="key" class="form-control" id="exampleInputName2" placeholder="关键字">
  </div>
...
//-->
					...
					<table  class="table table-hover">
						    ...
						    <tr align="center">
						 	<th   width="43"><span onClick="selectBox('reverse')">选</span></th>
								<th width="67" class="sort">用户ID</th>
								<th width="108" class="sort">推荐人/上级</th>
								<th width="108" class="sort">用户名</th>
								<th class="sort" width="99">用户密码</th>
								<th class="sort" width="67">用户余额</th>
								<th width="79">提现手续费</th>
								<th width="79">注册时间</th>
 ...
 $sql = 'select * from '.flag.'user where username like "%'.$_GET['key'].'%"  or   nickname like "%'.$_GET['key'].'%"    or  qq like "%'.$_GET['key'].'%"   order by ID desc';
 							}
							else{
									$sql = 'select * from '.flag.'user  order by ID desc , id desc';
 								}
						 ...
						 $username=str_replace($_GET['key'],"<font color=red> ".$_GET['key']."</font>",$row['username']);
						 $nickname=str_replace($_GET['key'],"<font color=red> ".$_GET['key']."</font>",$row['nickname']);
						 $qq=str_replace($_GET['key'],"<font color=red> ".$_GET['key']."</font>",$row['qq']);
 							...
 							?>
									...
									<td align=""><?=$row['ID']?></td>
									<td align=""><? if ($row['shangji']!='') {echo get_user('username',$row['shangji']).'编号:'.$row['shangji'].'';} else {echo "--";}?></td>
									<td align=""><?=$username?></td>
									<td align=""><?=$row['password']?></td>
									<td>￥<?=get_xiaoshu($row['rmb'])?></td>
									<td><?=$row['sxf']?> % </td>                                   
									<td><?=$row['date']?>  </td>                                   
									<td>
								      <a class="btn" href="rmbjl.php?uid=<? echo $row['ID']?>" >余额明细</a>
                                         <a class="btn" href="daili_edit.php?id=<? echo $row['ID']?>" >编辑</a>
                                <a class="btn" href="?act=del&id=<? echo $row['ID']?>" >删除</a>	
                                                            <a class="btn" href="userfc.php?uid=<? echo $row['ID']?>" >分成比例设置</a>  
                                                              </td>						</tr>
