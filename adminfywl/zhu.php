...
if(isset($_POST['提交'])){
	...
	$_POST['s_domain'] = array_filter($_POST['s_domain']);
  	//var_dump($_POST);
  	if(!$_POST['s_domain'])
    {
    	alert_back('请填写主域名!');
	...
	$_data['s_domain'] = trim($_POST['s_domain'][0]);
 	
  	$sql = 'update '.flag.'system set '.arrtoupdate($_data).' where id = 1';
	if(mysql_query($sql)){
		//alert_href('修改成功!','zhu.php');
	}else{
		alert_back('修改失败!');
	}
  
  
  	mysql_query("delete from ".flag."zhu");
  	
  	foreach($_POST['s_domain'] as $v)
    {	
      //echo "insert into ".flag."zhu values(null,'{$v}')";
    	mysql_query("insert into ".flag."zhu values(null,'{$v}')");
    }
...
<body>
	...
	<div class="col-sm-9 col-sm-offset-3 col-lg-10 col-lg-offset-2 main">			
		...
		<div class="row">
			<div class="col-lg-12">
<h1 class="page-header">主域名配置</h1>			</div>
		</div><!--/.row-->
						...
						<form class="form-horizontal" action="" method="post">
							<fieldset>
								...
								<?php
                              		...
                              		$result = mysql_query("select * from ".flag."zhu");
								...
								<div class="form-group">
									...
									<div class="col-md-9">
			                <input name="s_domain[]" type="text"    class="form-control" id="domain" value="<? echo $row['domain']?>" placeholder="请输入主域名">
		</div>
								</div>
						 ...
						 <div class="form-group">
									<label class="col-md-3 control-label" for="name">主域名</label>
									<div class="col-md-9">
			                <input name="s_domain[]" type="text"    class="form-control" id="domain" value="" placeholder="请输入主域名">
		</div>
								</div>
