...
if($_GET['act'] =='del'){
	$sql = 'delete from '.flag.'rmbjl where id = '.$_GET['id'].'';
	if(mysql_query($sql)){
		alert_href('删除成功!','rmbjl.php');
	}else{
		alert_back('删除失败！');
	}
}
...
/*如果要获取全部数值则使用下面代码*/
   $id= implode(",",$_POST['id']);
   $str="DELETE FROM ".flag."rmbjl where id in ($id)";
   mysql_query($str);
  echo "<script>alert('删除成功！');window.location.href='rmbjl.php';</script>";
}
}
		...
		<div class="row">
			<div class="col-lg-12">
				<div class="panel panel-default">
					<div class="panel-heading">余额记录</div>
...
//-->
					...
					﻿                    <form class="form-inline" action='' method='get'>
			                ...
			                <option  selected  value="?"  >全部</option>
		                  		 				                ...
		                  		 				                <option   <? if ($_GET['type']=='0') {echo "selected";}?>  value="?type=0"  >扣除 </option>
			                    			                 
		                  			                <option     <? if ($_GET['type']=='1') {echo "selected";}?>    value="?type=1"  >增加 </option>
  ...
  <button type="submit" class="btn btn-default">搜索</button>
					...
					<table  class="table table-hover">
						...
						<th   width="43"><span onClick="selectBox('reverse')">选</span></th>
 								<th width="62" class="sort">用户信息</th>
								<th width="131" class="sort">项目</th>
								<th width="86" class="sort">类型</th>
								<th width="76" class="sort"><span>前金额</span></th>
								<th class="sort" width="70">消费金额</th>
								<th class="sort" width="58">后金额</th>
								<th width="103">消费时间</th>
  ...
  if (isset($_GET['key'])) {
 $sql = 'select * from '.flag.'rmbjl where uid  = "'.$_GET['key'].'"  or   remark like "%'.$_GET['key'].'%"      order by ID desc , id desc';
 							}
							elseif (isset($_GET['uid'])) {
 $sql = 'select * from '.flag.'rmbjl where uid  =  '.$_GET['uid'].'   order by ID desc , id desc';
 							}
														elseif (isset($_GET['type'])) {
 $sql = 'select * from '.flag.'rmbjl where type  =  '.$_GET['type'].'   order by ID desc , id desc';
 							}
							
							else{
									$sql = 'select * from '.flag.'rmbjl  order by ID desc , id desc';
 								}
						 ...
						 {$type='<font color="green">增加</font>';}
 							?>
									...
									<td align=""><?=$row['remark']?></td>
									<td align=""><?=$type?></td>
									<td align="">￥<?=get_xiaoshu($row['qje'])?></td>
									<td align="">￥<?=get_xiaoshu($row['je'])?></td>
									<td align="">￥<?=get_xiaoshu($row['hje'])?></td>
									<td><?=$row['date']?>  </td>                                   
									<td>     <a class="btn" href="?act=del&id=<? echo $row['ID']?>" >删除</a>		</td>						</tr>
