...
if($_GET['act'] =='del'){
	$sql = 'delete from '.flag.'shipin where id = '.$_GET['id'].'';
	if(mysql_query($sql)){
		alert_href('删除成功!','detail.php');
	}else{
		alert_back('删除失败！');
	}
}
...
/*如果要获取全部数值则使用下面代码*/
   $id= implode(",",$_POST['id']);
   $str="DELETE FROM ".flag."shipin where id in ($id)";
   mysql_query($str);
...
<br>
  ...
  <div class="form-group">
    <a href="detail_add.php" class="btn btn-primary">新增</a>
					<a href="detail_pladd.php" class="btn btn-primary">批量新增</a>
...
//-->
					...
					<table  class="table table-hover">
						    ...
						    <tr align="center">
						        ...
						        <th data-field="name">所属分类</th>
						        <th data-field="name">视频标题</th>
						        <th data-field="name">视频地址</th>
						        <th data-field="name">视频主图</th>
  ...
  if (isset($_GET['key'])) {
 $sql = 'select * from '.flag.'shipin where name like "%'.$_GET['key'].'%"   order by sort desc , id desc';
 							}
							elseif (isset($_GET['cid'])) {
 $sql = 'select * from '.flag.'shipin where cid = '.$_GET['cid'].'   order by sort desc , id desc';
 							}
							else{
						 ...
						 $key1=str_replace($_GET['key'],"<font color=red> ".$_GET['key']."</font>",$row['i_name']);
 							...
 							?>
									...
									<td align=""><?=get_channel('name',$row['cid'])?></td>
									<td align="">
									 <?=$row['name']?>								</td>
									<td><a href="<? echo $row['url']?>"  target="_blank">	 <?=$row['url']?>	</a></td>
									<td> <a href="<? echo $row['image']?>"  target="_blank">	点击查看	</a></td>
									  
									<td>
								 
                                         <a class="btn" href="detail_edit.php?id=<? echo $row['ID']?>" >编辑</a>
