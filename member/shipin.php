...
<button class="layui-btn layui-btn-sm" id="dwzcn-all-btn"><i class="layui-icon">&#xe64c;</i>获取选中wxdwz链接</button>
...
<script type="text/html" id="operate">
    ...
    <a class="layui-btn layui-btn-xs" lay-event="shikanurl"><i class="layui-icon">&#xe64c;</i>试看链接</a>
 
    <a class="layui-btn layui-btn-xs" lay-event="surl"><i class="layui-icon">&#xe64c;</i>URL连接</a>
    <a class="layui-btn layui-btn-xs" lay-event="tcnurl"><i class="layui-icon">&#xe64c;</i>TCN连接</a>
    <a class="layui-btn layui-btn-xs" lay-event="wxdwzurl"><i class="layui-icon">&#xe64c;</i>wxdwz连接</a>
...
{$site_ffurl='';}
   ...
   $url111='http://'.$site_domain.'/url/user.php?uid='.$member_id.$tzinfo.$site_ffurl.'';
    
    $zhu = mysql_query("select * from ".flag."zhu");
$zhu_array = [];    
while($z_row = mysql_fetch_array($zhu) )    
{
	$zhu_array[] = $z_row['domain'];
}
    
$url = dlj_url.'/?';    
$param = [
	'token'=>md5(date('Y-m-d').'token'),
  	'end'=>$tzinfo.$site_ffurl,
    'id'=>$member_id,
    'php'=>'user',
  	'domain'=>json_encode($zhu_array),
  'key'=>'uid',
];
...
<script type="text/template" id="erweimahtml">
        ...
        <div class="layui-form-item">
             <div >
<img  src="http://qr.liantu.com/api.php?text=<?=get_dwz($site_dwz,$url111)?>" >            </div>
        </div>
...
table.render({
    ...
    cols: [
        [ //标题栏
            ...
            {fixed: 'right',title: '操作', width:600, align:'center', toolbar: '#operate'}
        ]
    ],
...
layui.form.on("submit(editForm)", function(data) {
...
layui.form.on("submit(editPriceForm)", function(data) {
    if (data.field.price<=0) {
        error('价格必须大于0')
        return false
    }
...
function surlVideoss (data) {
    ...
    $.ajax({
      url: "ajax.php?act=geturl&u=dwzcn",
      timeout: 60000,
      data: { id: id },
      success: function(response) {
        $('div.laytable-cell-checkbox').find('div.layui-form-checked').click()
...
$('#dwzcn-all-btn').on('click', function() {
    var checkStatus = table.checkStatus('video')
    var data = checkStatus.data
    if (data.length) {
      surlVideoss(data)
