...
<div class="layui-fluid child-body">
    <div class="layui-btn-group">
        <button class="layui-btn layui-btn-sm" > 提现记录</button>
                                                                           <?php 
  	$result = mysql_query("select sum(rmb-sxf) as total from ".flag."tx where zt = 1 and uid = {$member_id}");
                                   
  	$row = mysql_fetch_array($result);
                                   
	$total = $row['total']?:0;?>
       <button class="layui-btn layui-btn-sm" > 提现总额：<?php echo $total  ?></button>
    </div>
</div>
